...
class ImageRestorationNoisePretrainModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRestorationNoisePretrainModel, self).__init__(opt)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                pred = self.net_g(inp)
                noisy_=self.net_g.module.get_noise()
                if isinstance(pred, list):
                    pred = pred[-1]
                outs.append(pred.detach().cpu()[...,:ori_shape[0],:ori_shape[1]])
                noisy.append(noisy_[...,:ori_shape[0],:ori_shape[1]])
                i = j

            self.output = torch.cat(outs, dim=0)
            self.noisy=torch.cat(noisy,dim=0)
        self.net_g.train()
    ...
    def get_current_visuals(self):
        ...
        out_dict['lq'] = self.noisy.detach().cpu()#self.lq.detach().cpu()
