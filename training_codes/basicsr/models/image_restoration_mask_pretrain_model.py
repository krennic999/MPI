...
class ImageRestorationMaskPretrainModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRestorationMaskPretrainModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        # add mask
        mask=self.net_g.module.get_mask()
        preds=torch.mul(preds,1-mask)
        self.gt=torch.mul(self.gt,1-mask)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                inp,ori_shape=self.pad(self.lq[i:j])
                pred = self.net_g(inp)
                mask_=self.net_g.module.get_mask()
                if isinstance(pred, list):
                    pred = pred[-1]
                outs.append(pred.detach().cpu()[...,:ori_shape[0],:ori_shape[1]])
                mask.append(mask_[...,:ori_shape[0],:ori_shape[1]])
                i = j

            self.output = torch.cat(outs, dim=0)
            self.mask=torch.cat(mask,dim=0)
        self.net_g.train()
    ...
    def get_current_visuals(self):
        ...
        out_dict['lq'] = torch.mul(self.lq,1-self.mask).detach().cpu()
        out_dict['result'] = torch.mul(self.output.cuda(),1-self.mask).detach().cpu()
        if hasattr(self, 'gt'):
            out_dict['gt'] = torch.mul(self.gt,1-self.mask).detach().cpu()
        return out_dict
