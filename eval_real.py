...
if __name__ == "__main__":
    ...
    dataset=Get_Dataset(discrip='SIDD',path=path,path_gt=path_gt,
                        noise_type='real',sigma=0)
    dataloader=DataLoader(dataset,batch_size=1,shuffle=False)
    eval(dataloader,args,sigma=0)
    


    path='/PolyU/real/'
    path_gt='/PolyU/mean/'
    arg_path='./configs/Fill_m_polyu.yml'
    args=get_args(arg_path)
    dataset=Get_Dataset(discrip='PolyU',path=path,path_gt=path_gt,
                        noise_type='real',sigma=0)
    dataloader=DataLoader(dataset,batch_size=1,shuffle=False)
    eval(dataloader,args,sigma=0)


    path='/FMD/mixed_test/raw/'
    path_gt='/FMD/mixed_test/gt/'
    arg_path='./configs/Fill_m_polyu.yml'
    args=get_args(arg_path)
    dataset=Get_Dataset(discrip='FMD',path=path,path_gt=path_gt,
                        noise_type='real',sigma=0)
    dataloader=DataLoader(dataset,batch_size=1,shuffle=False)
    eval(dataloader,args,sigma=0)